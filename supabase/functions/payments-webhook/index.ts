...
async function handleSubscriptionUpdated(data: any, env: PaddleEnv) {
  ...
  if (item) {
    update.price_id = item.price.importMeta?.externalId || item.price.id;
    update.product_id = item.product?.importMeta?.externalId || item.product?.id;
  }
...
async function handleTransactionCompleted(data: any, env: PaddleEnv) {
  const { id, items, customData, subscriptionId } = data;
  // Only process one-time purchases (no subscription attached)
  ...
  // Only record if this is the lifetime price
  if (priceId !== 'premium_lifetime') return;
